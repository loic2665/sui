...
async fn test_full_node_event_read_api_ok() -> Result<(), anyhow::Error> {
    ...
    assert_eq!(events_by_sender.data[0].event, expected_event);
    ...
    let events_by_tx: EventPage = jsonrpc_client
        ...
        .await
        .unwrap();
    assert_eq!(events_by_tx.data.len(), 1);
    assert_eq!(events_by_tx.data[0].event, expected_event);
    ...
    assert_eq!(events_by_recipient.data[0].event, expected_event);
    ...
    let events_by_object: EventPage = jsonrpc_client
        ...
        .await
        .unwrap();
    assert_eq!(events_by_object.data.len(), 1);
    assert_eq!(events_by_object.data[0].event, expected_event);
    ...
    let events_by_module: EventPage = jsonrpc_client
        ...
        .await
        .unwrap();
    assert_eq!(events_by_module.data.len(), 1);
    assert_eq!(events_by_module.data[0].event, expected_event);
    ...
    assert_eq!(all_events.data.len(), 3);
